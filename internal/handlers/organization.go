...
// AdminTenantSummary represents live stats for a tenant in Super Admin
type AdminTenantSummary struct {
	...
	WhatsAppLines int64     `json:"whatsapp_lines"`
	OwnerEmail    string    `json:"owner_email"`
}

// GetAdminOverview returns dynamic real-time platform statistics for Super Admin
func (a *App) GetAdminOverview(r *fastglue.Request) error {
	var orgs []models.Organization
	if err := a.DB.Order("created_at DESC").Find(&orgs).Error; err != nil {
		a.Log.Error("Failed to list organizations for admin overview", "error", err)
		return r.SendErrorEnvelope(fasthttp.StatusInternalServerError, "Failed to load overview", nil, "")
	}
	...
	tenantSummaries := make([]AdminTenantSummary, len(orgs))
	for i, org := range orgs {
		...
		a.DB.Model(&models.UserOrganization{}).Where("organization_id = ?", org.ID).Count(&seatsCount)
		a.DB.Model(&models.WhatsAppAccount{}).Where("organization_id = ?", org.ID).Count(&wabaCount)
		...
		if org.Settings != nil {
			if pt, ok := org.Settings["plan_tier"].(string); ok && pt != "" {
				planTier = pt
			}
			if st, ok := org.Settings["status"].(string); ok && st != "" {
				status = st
			}
		}
		...
		if status == "active" {
			estimatedMRR += float64(planPrice)
		}

		var owner models.User
		a.DB.Joins("JOIN user_organizations ON user_organizations.user_id = users.id").
...
func (a *App) CreateOrganization(r *fastglue.Request) error {
	...
	// Seed sample onboarding flow for the new organization
	if err := database.SeedSampleFlowForOrg(tx, org.ID, &userID); err != nil {
		tx.Rollback()
